---
name: bcar-admin
description: bcar 운영 스크립트(tools/bcar_admin.py)로 dev 시드 데이터 복사, 유저 시트 targetSite·quota 채우기, _control 플래그 조회/변경을 한다. "dev에 데이터 채워", "prod 데이터 dev로", "시드", "seed-dev", "유저 시트", "targetSite", "quota 채워", "stopDetail", "체인 멈춰", "_control", "체인 카운터 리셋"을 언급하면 명시적으로 부르지 않아도 사용한다. 앱 잡(collect-draft/collect-detail/assign-cars) 실행은 이 스킬이 아니라 AWS Batch로 제출한다.
---
